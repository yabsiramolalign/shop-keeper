                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Text(_billController.todaySummary.value.total
                              .toStringAsFixed(2)),
                        ],
                      ),
                      Row(
                        children: [
                          ...
                          Text(_billController.todaySummary.value.income
                              .toStringAsFixed(2)),
                        ],
                      ),
                      Row(
                        children: [
                          ...
                          Text(_billController.todaySummary.value.depth
                              .toStringAsFixed(2)),
                        ],
                      ),
                    ],
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            SizedBox(height: 5),
                                            Text(
                                              "Kg price: ${item.kgPrice.toStringAsFixed(2)}",
                                              style: TextStyle(
                                                color: Colors.grey,
                                                fontSize: 14,
                                              ),
    ...
    try {
      for (final item in controller.itemList) {
        ...
        if (textController?.text != null && textController!.text.isNotEmpty) {
          ...
          if (buyAmount > 0) {
            final sellItem =
                SellItem(item.name, item.kgPrice, item.unitPrice, buyAmount);
      ...
      if (sellItems.isNotEmpty) {
        final bill = Bill(
            DateFormat.yMMMd().format(DateTime.now().add(Duration(days: 0))),
            sellItems,
            '',
            'Paid',
            0);
            ...
            child: SizedBox(
              ...
              child: SingleChildScrollView(
                child: DataTable(
                  ...
                  rows: [
                    for (int index = 0;
                        index < widget.bill.sellItems.length;
                        index++)
                      DataRow(
                        cells: [
                          DataCell(Column(
                            children: [
                              ...
                              Text(widget.bill.sellItems[index].unitPrice == 0
                                  ? widget.bill.sellItems[index].kgPrice
                                      .toString()
                                  : widget.bill.sellItems[index].unitPrice
                                      .toString())
                            ],
                          )),
                          DataCell(Text(widget.bill.sellItems[index].buyAmount
                              .toStringAsFixed(2))),
                          DataCell(Text(widget.bill.sellItems[index].itemTotal
                              .toStringAsFixed(2)))
                        ],
                      )
                  ],
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text('Total:'),
                      Text(
                        widget.bill.fullTotal.toStringAsFixed(2),
                        style: TextStyle(
                  ...
                  DropdownButton(
                    ...
                    items: [
                      ...
                      DropdownMenuItem(
                          child: Text('Not Paid'), value: 'Not Paid'),
                      DropdownMenuItem(
                          child: Text('Half Paid'), value: 'Half Paid'),
                    ],
                    onChanged: (value) {
                      setState(() {
                        status = value!;
                  ...
                  status == 'Not Paid' || status == 'Half Paid'
                      ? TextField(
            ...
            onPressed: () {
              if (widget.bill.payedStatus == 'Not Paid') {
                if (buyerController.text.trim().isNotEmpty &&
                  ...
                  widget.textControllers.values
                      .forEach((controller) => controller.clear());
                  _billController
                      .fetchBills(DateFormat.yMMMd().format(DateTime.now()));
                ...
                if (buyerController.text.trim().isNotEmpty &&
                    halfPayController.text.trim().isNotEmpty) {
                  widget.bill.buyerName = buyerController.text;
                  widget.bill.payedAmount =
                      double.parse(halfPayController.text);
                  _salesController.updateSalesData(widget.bill.sellItems);
                  _billController.addBill(widget.bill);
                  widget.textControllers.values
                      .forEach((controller) => controller.clear());
                  ...
                  Get.snackbar(
                      'Error', 'Buyer and Paid amount cannot be empty!',
                      snackStyle: SnackStyle.FLOATING,
                      backgroundColor: Colors.red,
                      colorText: Colors.white);
                }
